...
Campaign capability authorization is data-driven through `config/progression.json`. Each Act 1 tier is a two-clear chapter: its Hybrid mission opens Swarm and Armored contracts, and clearing either role contract opens the next Hybrid plus the chapter award. Those awards are Consumable Bay 1 plus a Shield Overcharge sample (Mission 1), Mini plus a choose-one Certified mini requisition (Mission 2), EMP plus an Armor Sealant sample/supply authorization (Mission 3), Primary B in Swap mode plus a choose-one Certified primary requisition (Mission 4), Damage Overcharge plus a sample (Mission 5), named-hull licenses and Bulwark (Mission 6), and Consumable Bay 2 (Mission 7). Locked Armory hardpoints remain visible with their requirement. Existing saves keep collected inventory; Test Arsenal temporarily bypasses authorization without changing completion records.

Mission supplies are persistent, finite stock rather than per-mission freebies. Shield Overcharge, Armor Sealant, and Damage Overcharge unlock during Act 1, receive one first-unlock sample, can be bought from the Ledger at provisional loss-leading prices, and are assigned to two visible Armory bays. Shield Overcharge restores 40% and permits 25% temporary shield overflow; Armor Sealant restores armor, permits 20% temporary overplate, and repairs or temporarily overplates hull-only builds; Damage Overcharge grants +40% primary damage for seven seconds. Scripted shield and armor pickups call the same runtime effects. In-flight HUD buttons and keys `1`/`2` consume stock on use, and the debrief reports supplies spent plus replacement-cost net. Test Arsenal and the DEV menu can refill all supply lockers.
