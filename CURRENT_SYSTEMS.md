...
Mission replays are intentionally stable. Completed mission count remains useful for records, mission numbering, market refreshes, and bulletin cadence, but it does not scale combat difficulty. Enemy pressure can still increase during a mission through elapsed-time difficulty and explicit level or enemy configuration. Levels can now author enemy projectile profiles and weighted attack patterns so chip fire, standard bolts, heavy warning shots, and boss hazards have explicit per-shot damage, speed, size, and visual treatment. Legacy flat enemy bullet fields remain valid for baseline missions.

Named hulls shape loadouts through shield, armor, aux, mini, and second-bay modifiers. One-primary builds receive a visible shield focus bonus while carrying a second primary applies a visible defense strain unless hull and engineering choices mitigate it. The Armory stat popup reports mission-ready layer stats, installed defense item internals, and selected-mission projectile damage estimates for debugging live loadouts without reducing inventory space. Existing combat UI exposes health layers, mission objective state, cargo, selected support capability, equipped primary weapon icons, active primary bay or Dual Fire state, swap cooldown, and mini weapon state.
...
The campaign has a core progression plus variants and lab-style encounters. Threat Mix variants exist for every player-facing `level*.json` mission and are exposed through the existing mission carousel for armor-class testing without replacing the original baseline missions. Older patrol and skirmish files remain in the level directory for reference and compatibility, but the current player-facing loop is driven by the implemented mission select and unlock state.
