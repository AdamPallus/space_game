...
- A test-only Flocking Lab reuses the Chorus fleet to exercise local separation, same-school cohesion/alignment, predictive projectile-lane avoidance, and conductor-led coordination/collapse. Flocking ships can also author a scored departure window: survivors withdraw, count against destruction rating, and stop firing during egress instead of accumulating forever.
- The player-approved test-only `Broodwake Lab: Living Current` applies local awareness to Verdant organisms. Loose currents keep personal space, then choose the highest-priority living producer and arrange a player-facing defensive crescent around it. Nearby wounds propagate an alarm that tightens the guard and draws visible signaling filaments; losing every protected producer briefly turns the survivors into a rushing rage. The lab uses the older `05_building_a_colony_loop_1.ogg` track instead of the newer missions' repeated battle soundtrack. Its successful behavior test now feeds the full-sized `The Living Current` campaign candidate.
- The player-facing campaign runs from Mission 1 through Mission 8 / Last Light, then continues through five player-validated slices and two inserted candidates: Crossed Claims (hybrid), Processional (Chorus swarm), Collateral Choir (evasive Chorus/Tithe hybrid), Repossession (Tithe armor), The Green Signal (Verdant ecology), The Living Current (Verdant shoal), and Return Address (Verdant interception). Collateral Choir appears after Processional; The Living Current appears after The Green Signal. The Living Current's full encounter is player-approved but still does not gate Return Address until its route status is deliberately chosen. After Return Address, the three-mission experimental Parallax act unlocks sequentially: A Strange Angle, False Position, and Proof of Life. The Parallax is playable combat research, not locked story canon. Missions 9–11 and the remaining old Act 2/3 encounter scripts are hidden from the normal board; `?devActs=1` exposes and unlocks them only for agentic regression access.
- The Parallax has a generated black-glass/crystal fleet and three generated void backgrounds. Its shared `parallax` AI places group members into exact arc, wall, orbit, braid, spear, and mirror formations; opens a group-wide aperture around predicted projectile lanes; recomputes slots after losses; supports delayed or reflected player tracking; and can withdraw surviving light geometry after a scored combat window. Cyan/violet link lines and amber pulses make the shared calculation visible, while a runtime audit reports groups, formations, spacing, overlaps, protection, departures, and avoidance events.
...
- Search, sort, and filter controls for armory inventory, item archive, and Ledger sell views. The Armory is a ship-first workspace: compact hardpoints and supply bays remain visible, clicking a hardpoint opens a temporary inventory drawer, and on desktop that drawer takes a dedicated right column so every ship hardpoint remains clickable. Immediate item popups replace delayed browser tooltips, chapter-award offers derive stats from each offered item's real slot, and optional comparison shows installed and candidate items together. `Show Stats` remains a separate player-facing popup.
- Mini weapons with rarity-scaled output/effects, defense loot with rarity-scaled armor-class/shield strength, named hulls, second-primary swapping, one-primary primary-damage focus bonuses, aux engineering upgrades, and campaign-earned dual-fire tiers for compatible loadouts. Last Light grants Tier 1; either claimant mission grants Tier 2; Return Address or both claimants grants Tier 3; Return Address plus both claimants grants Tier 4. Carrying a second primary applies a visible per-weapon primary-damage strain. Impulse budget benefits kinetic and plasma weapons through different tradeoffs: kinetic turns impulse into speed-fed damage, while plasma turns impulse into larger, slower, higher-damage orbs. Non-rapid paired kinetic drivers add a factory recoil reserve before that same impulse calculation, so Twin Driver and Oathpair trade cadence for materially heavier hits while preserving item, aux, and supply impulse synergy. Shield layers take raw projectile damage before armor; armor class mitigates only hits that reach armor, uses the best installed armor class, and does not add duplicate armor-class values from multiple plates. Plasma burn stacks for three seconds from repeated hits and ignores armor-class subtraction while burning armor, but it still drains shields, armor, and hull in order and armor takes reduced burn.
...
- The normal-play gold path is now data-driven and idempotent. Mission 6 grants a choose-one Prototype defensive system, Mission 7 a choose-one Prototype role weapon, and Last Light grants separate Pre-Founding primary and survival commissions. Last Light's control choice rolls a Pre-Founding standard EMP or Bulwark instead of guaranteeing Orphan Signal; Orphan keeps its full power and remains obtainable from later gold commissions. Crossed Claims, Processional, Repossession, The Green Signal, and Return Address each add a role-weighted Pre-Founding choice on first clear. Offers are rolled once into the save, avoid already-owned bases when their pool permits, retain mission/role provenance, fill an open compatible hardpoint when possible, and otherwise enter Armory storage.
- Act 2 uses its own salvage rarity table: ordinary field drops remain uncommon, transports have a 10% gold chance, captains 18%, minibosses are guaranteed Prototype-or-better with a 20% gold chance, and bosses are Prototype-or-better with a 30% gold chance. Guaranteed commissioning rewards are separate from boss salvage.
...
- Repossession proved that random sub-gold salvage is not worth combat risk once gold gear is required. Slice C now supplies Act 2 rarity floors and separate guaranteed gold commissions without special-casing a single encounter; fresh-save runs accepted the resulting acquisition pace.
- The revised chapter pacing and supply-credit redesign are player-approved. Repeated boosters enabled a Last Light clear at meaningful debt, including a negative balance and forced inventory sales, which is the intended loss-leading insurance behavior.
- The campaign progression spine is player-accepted. A later unassisted novice run reached Mission 7 without using the marketplace, understood swarm/armor roles, and enjoyed the capability and weapon-swap progression. That run exposed Oathpair Driver and the underlying non-rapid dual-kinetic pattern as too weak; paired kinetics now receive a shared archetype impulse reserve, and Oathpair's unique reserve is strengthened without rerolling existing saved affixes.
...
- `Collateral Choir` is an authorized post-Last-Light campaign candidate and is awaiting its first human playtest. It unlocks after Processional, appears immediately before Repossession on the board, grants ordinary Act 2 salvage but no guaranteed first-clear commission, and is not a prerequisite for any existing mission. Phase 8 remains separate brainstorm/spec territory. The old Act 3 missions remain hidden experiments. The new Parallax act is an explicit creative-combat exception, while final canonical framing still waits for the story/economy setup and The Fork. The Fork is a story choice and bridge, not Act 3 itself.
- `The Living Current` is player-approved as a full encounter. Its 80-second route from loose currents through Bloomcaller centers, splitting Seedcarriers, a Broodmother handoff, and The Broodwake appears between The Green Signal and Return Address. It uses ordinary Act 2 salvage plus a Prototype field pod and miniboss/boss drop opportunities, grants no guaranteed commission, and does not yet gate Return Address because required-versus-optional routing remains an explicit future choice.
- `A Strange Angle`, `False Position`, and `Proof of Life` are an owner-authorized experimental Act 3 combat candidate. They unlock sequentially after Return Address, use varied existing placeholder music, new Parallax art/backgrounds, and the new formation/avoidance AI, and target stronger gold or early Heirloom builds. Their provisional contact language does not settle the Phase 8–10 story arc. Automated browser stress checks established bounded populations and responsive rendering; balance, boss-phase feel, plasma/chaff interaction, and eventual canonical status await Adam's real-loadout playtest.
...
- Generated asset passes: UI chrome, item icon pack, combat fleet art, Act 3 invasion art, Parallax geometric fleet art, and mission backgrounds.
...
- Loot depth pass (Phase 6): per-instance affix magnitude rolls (`roll` ranges in `item_pool.json`), effect-trace potency tiers, a stored `rollQuality` that scales item value and drives a tooltip roll-quality bar, pre-Founding 3-affix rolls, aux abilities with per-item rolled potency knobs (`auxPotency`/`auxRoll`) replacing the retired `auxPower` investment, and a `balance_report.js` magnitude-roll DPS spread section. The later dual-fire-only credit ladder is now retired too; old purchases receive a one-time migration credit based on its final listed prices before campaign records determine the earned tier.
...
- Paired kinetic calibration pass: `dual` kinetic frames receive a factory recoil impulse reserve through the normal kinetic damage model, Oathpair's gold unique property adds a deeper reserve, tooltips disclose the paired contribution, and the balance report protects Twin/Stitch and Oathpair/Needleloom damage-per-shot, raw-DPS, and plated-target relationships.
...
- Campaign supply-credit pass: four config-driven mission supplies unlock through Act 1; they are free to equip and charged only when activated, settle after every outcome, and may create debt. Negative net settlement renders red. Each bay earns independent charge authorization through completed chapters. Shield Overcharge sets a persistent 125%, Armor Sealant restores installed armor to 100%, and Redline/Sustained Impulse use the real projectile-impulse model. Their lockouts match effect duration; distinct overlaps multiply to 2.8x while same-type reactivation refreshes. Combat uses real icon-led activation buttons, duration/recharge sweeps, and separate around-ship effect arcs. First-clear samples and DEV refills are fee waivers; redundant field pickups become extraction-only credit claims. The Ledger no longer sells supplies. The Armory is ship-first with a temporary inventory drawer, immediate custom popups, optional comparison, and validator-enforced removal of native delayed tooltips.
...
- Parallax experimental-act pass: three sequential post-Return-Address missions, a shared exact-formation/projectile-avoidance AI, scored chaff departure, linked-geometry rendering and audit telemetry, radial geometric bosses, three generated backgrounds, varied placeholder music, and existing Heirloom first-clear plumbing.
